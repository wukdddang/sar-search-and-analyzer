...
| | | | | AOI 그리기 툴바 | DRAW | Layer | SAR_SRH_DRAW | — | 지도 우상단 툴바 | - 도구: bbox / polygon / 지역선택 / SHP업로드 / 전체화면<br>- 활성 도구 하이라이트<br>- bbox·polygon 그리기 중 ESC로 취소 가능<br>- 완료 시 AOI 자동 적용 + 자동 검색 트리거 |
| | | | | SHP 업로드 | UPLOAD | Layer | SAR_SRH_UPLOAD | — | SHP zip 드래그앤드롭 모달 | - `.zip` 단일 파일만 허용 (확장자·MIME 검증)<br>- 최대 50MB<br>- `shpjs`로 브라우저 파싱 → GeoJSON 변환<br>- 지도에 즉시 미리보기(파란색 폴리곤)<br>- 좌표계 EPSG:4326 또는 prj에서 자동 감지<br>- [AOI로 사용] 버튼 → 모달 닫고 검색 트리거<br>- [공용 데이터셋으로 저장] → SAR_PDS_FORM 으로 전환 |
