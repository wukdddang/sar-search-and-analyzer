...
- **AOI 드로잉 툴바**: `bbox`, `polygon`, `지역 선택`, `주소 검색` (VWorld 기반, [20. VWorld API 통합](./20-vworld-integration.md)), `전체 화면` 5개 도구. `SHP 업로드`는 보조 경로로 별도 메뉴에만 유지.
...
## 6. 공공데이터/SHP 업로드 플로우 (DEPRECATED)

> **본 플로우는 2026-04-24 이후 보조 경로로 격하됨.** 연속지적도(필지) 폴리곤은 [20. VWorld API 통합](./20-vworld-integration.md)의 주소 검색 기반 질의 API로 대체. SHP 업로드는 비지적 경계(기상청 특보 영역, 행안부 특수 행정구역 등)가 필요할 때만 재개.
...
| 엔드포인트 | 용도 | 권한 |
|-----------|------|------|
| `GET /api/v1/geo/search` | 주소 → 지번 후보 목록 (VWorld 프록시) | `viewer`+ |
| `GET /api/v1/geo/parcel/{pnu}` | PNU → 필지 폴리곤 (VWorld 프록시 + 캐시) | `viewer`+ |
| `POST /api/v1/aois/from-parcel` | PNU 폴리곤을 사용자 AOI로 저장 | `viewer`+ |
| `POST /api/v1/aois/from-parcels` | 여러 PNU 묶어 AOI(멀티폴리곤) 저장 | `viewer`+ |
| `POST /public-datasets` _(deprecated)_ | SHP zip 업로드 (multipart) — 1차 범위 제외 | `downloader`+ |
| `GET /public-datasets` _(deprecated)_ | 공공데이터셋 목록 | 인증 |
| `GET /public-datasets/{id}` _(deprecated)_ | 상세 (GeoJSON geom 포함) | 인증 |
