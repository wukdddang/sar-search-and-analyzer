...
| | UC-SRH05 | ~~SHP 업로드로 AOI 지정~~ _(DEPRECATED, §[20](./20-vworld-integration.md) 참조)_ | 보조 경로로만 유지 — 비지적 SHP 데이터용. | ⚠️ | ⚠️ | ⚠️ |
| | UC-SRH05b | **주소 검색으로 AOI 지정** | 주소/지번 입력 → VWorld 후보 → 필지 폴리곤을 AOI로 사용. | ✔️ | ✔️ | ✔️ |
| | UC-SRH06 | ~~공공데이터셋으로 AOI 지정~~ _(DEPRECATED, §[20](./20-vworld-integration.md))_ | SHP 업로드 의존 기능이라 1차 범위 제외. | ⚠️ | ⚠️ | ⚠️ |
...
| 구분 | UC ID | 유즈케이스명 | 설명 | Viewer | Downloader | Admin |
|------|-------|-------------|------|:------:|:----------:|:-----:|
| 공공데이터 (DEPRECATED) | UC-PDS01~09 | _1차 스프린트 범위 제외_ | 연속지적도는 VWorld API로 대체 ([20](./20-vworld-integration.md)). 비지적 SHP가 필요한 경우에만 재개. | ⚠️ | ⚠️ | ⚠️ |
...
| | UC-AOI03 | ~~SHP로 AOI 생성~~ _(DEPRECATED, §[20](./20-vworld-integration.md))_ | 보조 경로 (비지적 SHP 용). | ❌ | ❌ | ⚠️ |
| | UC-AOI03b | **주소/PNU로 AOI 생성** | 주소 단건 또는 PNU CSV 일괄 → VWorld 조회 → 멀티폴리곤 AOI. | ❌ | ❌ | ✔️ |
